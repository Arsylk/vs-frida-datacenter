...
function hookLocalStorage(fn?: (key: string) => string | undefined) {
    ...
    ClassLoader.perform(() => {
        if (!Cocos2dxLocalStorage && (Cocos2dxLocalStorage = findClass('org.cocos2dx.lib.Cocos2dxLocalStorage'))) {
            hook(Cocos2dxLocalStorage, 'getItem', { replace: fn ? ifKey(fn) : undefined, logging: { multiline: false } });
        }
    });
}
