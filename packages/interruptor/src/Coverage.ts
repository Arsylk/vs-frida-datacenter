...
export class CoverageAgent {
    ...
    out: any = null;
    stops: any = { count: Number.POSITIVE_INFINITY };
    ...
    static padStart(data: string, length: number, pad: string): string {
        ...
        const result = pad.repeat(fullPads) + pad.substring(0, partialPadLength) + data;
        return result;
    }
    /**
    ...
    static write32le(address: NativePointer, value: number): void {
        for (let i = 0; i < CoverageAgent.BYTES_PER_U32; i += 1) {
            // tslint:disable-next-line:no-bitwise
            const byteValue = (value >> (CoverageAgent.BITS_PER_BYTE * i)) & CoverageAgent.BYTE_MASK;
            address.add(i).writeU8(byteValue);
        }
    ...
    emitHeader(events: number) {
        ...
        this.emit(
            CoverageAgent.convertString(
                `Module Table: version 2, count ${this.interruptor.modules.length}\n`,
            ),
        );
