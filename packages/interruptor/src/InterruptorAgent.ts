import { CoverageAgent } from './Coverage.js';
import { F, type InterruptSignatureMap, L, type SyscallHookMap, SyscallInfo } from './types.js';
import type { IStringIndex } from './IStringIndex.js';
import { SVC } from './LinuxAarch64Syscalls.js';
...
export class InterruptorAgent implements IStringIndex {
     ...
     */
    constructor(
        pOptions: InterruptorAgentConfig,
        pDoFollowThread: any = null,
        pInterrupts: InterruptSignatureMap | null = null,
    ) {
        this.uid = CTR++;
        this.emulator = false;
    ...
    parseOptions(pConfig: any): void {
        for (const k in pConfig) {
            switch (k) {
                ...
                case 'followThread':
                    this.followThread =
                        typeof pConfig.followThread !== 'boolean' ? false : pConfig.followThread;
    ...
    getModuleList(pFilters: ScopeFilter[], pSrcList: Module[] = [], pList: string[] = []): string[] {
        ...
        pFilters.map((vFilter: ScopeFilter) => {
            switch (typeof vFilter) {
                case 'string':
                    modules.map((x) => {
                        if (x.name === vFilter) list.push(x.name);
                    });
                    break;
                case 'function':
                    modules.map((x) => {
                        if (vFilter(...[x])) list.push(x.name);
                    });
                    break;
                case 'object':
                    if (Array.isArray(vFilter)) {
                        ...
                        modules.map((x) => {
                            if (vFilter.exec(x.name) != null) list.push(x.name);
                        });
        ...
        switch (typeof pSyscalls) {
            case 'string':
                this.interrupts.syscalls.map((x) => {
                    if (x[1] === pSyscalls) list.push(x[SyscallInfo.NUM]);
                });
                break;
            case 'function':
                this.interrupts.syscalls.map((x) => {
                    if (pSyscalls(...[x])) list.push(x[SyscallInfo.NUM]);
                });
                break;
            case 'object':
                if (Array.isArray(pSyscalls)) {
                    pSyscalls.map((sVal) => {
                        switch (typeof sVal) {
                            case 'string':
                                this?.interrupts?.syscalls?.map((x) => {
                                    if (x[SyscallInfo.NAME] === sVal) list.push(x[SyscallInfo.NUM]);
                                });
                                break;
                            case 'number':
                                this?.interrupts?.syscalls?.map((x) => {
                                    if (x[SyscallInfo.NUM] === sVal) list.push(x[SyscallInfo.NUM]);
                                });
                                break;
                            case 'object':
                                this?.interrupts?.syscalls?.map((x) => {
                    ...
                    this.interrupts.syscalls.map((x) => {
                        if (pSyscalls.exec(x[1]) != null) list.push(x[0]);
                    });
                } else {
                    this.interrupts.syscalls.map((x) => {
                        list.push(x[SyscallInfo.NUM]);
                    });
                }
                break;
            default:
    ...
    private _filterSyscallScope(): void {
        ...
        if (this.scope.syscalls != null) {
            ...
            if (scope.exclude) {
                ...
                scope.isExcluded = (x: number) => {
                    return syscalls.indexOf(x) > -1;
                };
            } else {
    ...
    isTrackCoverage(): boolean {
        return this.coverage?.enabled === true;
    }
    ...
    start(pTID = -1) {
        ...
        if (this.debug.scope) {
            const D = DebugUtils;
            D.printModuleScopes(this);
            D.printSyscallScopes(this);
        }
        ...
        console.warn(
            `${this.output.indent}[INTERRUPTOR][STARTING] Tracing thread ${tid} [${this._tids.join(',')}]`,
        );
        this._tids.push(tid);
        ...
        pSyscallList.map((sc) => {
            ...
            for (let i = 0; i < sc[3].length; i++) {
                ...
                if (arg.l != null && arg.l === L.DSTRUCT) {
                    if ((types as any)[arg.f] == null) (types as any)[arg.f] = 1;
                    else (types as any)[arg.f]++;
                }
            }
