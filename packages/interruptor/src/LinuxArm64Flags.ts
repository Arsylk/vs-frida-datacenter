import type { ErrorCodeList, ErrorCodeMapping } from './types.js';
import { IStringIndex } from './IStringIndex.js';
import type { KernelConstMapping, KernelEnum } from './types.js';
...
export const X = {
    ...
    LINKAT: (f: number) => {
        if (f === AT_.AT_SYMLINK_FOLLOW[0]) return 'AT_SYMLINK_FOLLOW';
        return 0; // no flag
    },
    MLOCK: (f: number) => {
        if (f === MLOCK.MLOCK_ONFAULT[0]) return 'MLOCK_ONFAULT';
        return 0; // no flag
    },
    PKEY_ACL: (f: number) => l(f, PKEY),
    RUSAGE: (f: number) => l(f, RES),
    RES: (f: number) => l(f, RES),
    RWF: (f: number) => l(f, RWF),
    SECCOMP: (f: number) => l(f, SECCOMP),
    SECCOMP_FLAGS: (f: number, cmd: any) => f,
    MEMBARRIER_CMD: (f: number) => l(f, MEMBARRIER_CMD),
    MEMBARRIER_FLAG: (f: number) => l(f, MEMBARRIER_FLAG),
    ACCESS_FLAGS: (f: number) =>
        stringifyBitmapArr(f, {
            AT_SYMLINK_NOFOLLOW: [0x100],
            AT_NO_AUTOMOUNT: [0x800],
            AT_EMPTY_PATH: [0x1000],
        }),
    EPOLL_EV: (f: number) => stringifyBitmapArr(f, EPOLL_EV),
    SPLICE: (f: number) => stringifyBitmapArr(f, SPLICE),
    ITIMER: (f: number) => stringifyBitmapArr(f, ITIMER),
    SYNC_FILE: (f: number) => stringifyBitmapArr(f, SYNC_FILE),
    EPOLL_CTL: (f: number) => stringifyBitmapArr(f, EPOLL_CTL),
    EPOLL_FLAG: (f: number) =>
        stringifyBitmapArr(f, {
            EPOLL_CLOEXEC: [O_.O_CLOEXEC],
        }),
    PRCTL_OPT: (f: number) => l(f, PR_.OPT),
    CLONE: (f: number) => stringifyBitmapArr(f, CLONE),
    CLK: (f: number) => l(f, CLOCK),
    SCHED: (f: number) => l(f, SCHED_),
    SEEK: (f: number) => l(f, SEEK_),
    INOTIFY_FLAGS: (f: number) => l(f, INOTIFY_FLAGS),
    INOTIFY_MASK: (f: number) => l(f, INOTIFY_MASK),
    FUTEX_OPE: (f: number) => l(f, FUTEX),
    PTRACE: (f: number) => l(f, PTRACE_),
    PTRACE_DATA: (f: number) => l(f, PTRACE_),
    NODMODE: (f: number) => {
        // todo parse dev
    ...
    FLOCK: (f: number) => l(f, LOCK),
    FALLOC: (f: number) => l(f, FALLOC),
    IOPRIO_WHICH: (f: number, cmd: any) => l(f, IOPRIO_WHO),
    PERSO: (f: number) => l(f, PERSO),
    TYPEID: (f: number) => l(f, K),
    XATTR: (f: number) => ['default', 'XATTR_CREATE', 'XATTR_REPLACE'][f],
    UNLINK: (f: number) => l(f, { AT_REMOVEDIR: AT_.AT_REMOVEDIR }),
    PIPE_FLAG: (f: number) => {
    ...
    FNCTL: (f: number) => l(f, F_),
    FCNTL_RET: (f: number, cmd: any) => {
        switch (cmd) {
            case F_.F_GETFL:
                return X.O_MODE(f);
            default:
    ...
    DEL_KEXT: (f: number) => stringifyBitmapArr(f, { O_NONBLOCK: O_.O_NONBLOCK, O_TRUNC: O_.O_TRUNC }),
    SIG_FLAGS: (f: number) => l(f, SIG_FLAG),
    SIG: (f: number) => l(f, S),
    PF: (f: number) => l(f, PF_),
    SOCK: (f: number) => stringifyBitmapArr(f, SOCK_),
    MOUNT_FLAG: (f: number) => stringifyBitmapArr(f, MOUNT),
    MADV: (f: number) => l(f, MADV_),
    MCL: (f: number) => l(f, MCL_),
    MAP: (f: number) => stringifyBitmapArr(f, MAP_),
    MS: (f: number) => l(f, MS_),
    ERR: (f: number) => {
    ...
    ATTR: (f: number) => f,
    UMASK: (f: number) => stringifyBitmapArr(f, S_),
    O_FLAG: (f: number) => stringifyBitmap(f, O_),
    O_MODE: (f: number) => stringifyBitmap(f, O_),
    F_MODE: (f: number) => stringifyBitmapArr(f, FMODE),
    UMOUNT: (f: number) => stringifyBitmapArr(f, MNT_),
    MFD: (f: number) => stringifyBitmapArr(f, MFD),
    MPROT: (f: number) => {
        if (f === PROT_NONE) return 'PROT_NONE';
