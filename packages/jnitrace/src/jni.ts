...
const JNI: { [key: string]: JniMethodDefinition } = {
...
function asFunction(base: NativePointer, def: JniMethodDefinition) {
    const ptr = base.add(def.offset * Process.pointerSize);
    const fn = new NativeFunction<any, any>(ptr.readPointer(), def.retType, def.argTypes);
    return fn;
}

export { JNI, asFunction, JniMethodDefinition, jMethodID };
